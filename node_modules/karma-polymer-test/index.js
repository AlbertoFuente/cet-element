...
createProxy = function(path, config, proxy) {
  'use strict';
  var base = config.basePath.replace(/\\/g,'/');
  path = path.replace(/\\/g,'/');
  proxy = (proxy && proxy.replace(/\\/g,'/') || path.substr(path.lastIndexOf('/')));
  path = 'http://' + config.hostname + ':' + config.port + '/base' + path.replace(base, '');
...
initPolymerTest = function (injector, config, logger, webServer) {
  ...
  config.files.unshift(createPattern(bowerDir + '/webcomponentsjs/webcomponentsjs.min.js', true));
  ...
  createProxy('', config, '/bower_components/test_component');
